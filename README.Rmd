...
census_api_key("e372e284c5381fe44e39f649bb20dac827bd3a06")
```
Function
...
Before running the function script a Census API key is required: https://api.census.gov/data/key_signup.html.  Data collected will be 2016 American Community Survey (ACS) estimates. 

tractND will extract census estimates at the tract level, transform the variables, and then perform a Principal Component Analysis.  
...
kable(NDI)
```
The output variable 'PC1' is the deprivation index score for each corresponding census tract (CT) in the analysis.

Higher index scores represent higher deprivation.  Here is the distribution of deprivation across Onondaga County, NY 

``` {r, echo = TRUE}
plot(ggplot(NDI, aes(PC1)) + geom_histogram() + theme_classic())
```
...
plot(ggplot(NDI, aes(reorder(Tract, -PC1), PC1)) + geom_col(aes(fill = type)) + coord_flip() +
...
plot(ggplot() + geom_sf(data = Map, aes(fill = PC1)) +
...
plot(Map %>% filter(as.numeric(TRACTCE) < 6104) %>%
...
plot(Map2 %>% filter(as.numeric(TRACTCE) < 980000) %>%  ## exclude Everglades Nat'l Park
ggplot() + geom_sf(aes(fill = PC1)) +
  theme_minimal() + scale_fill_viridis()+
  labs(fill = "Index", caption = "Source: US Census ACS 2016 estimates")+
...
plot(ggplot() + geom_sf(data = Map3, aes(fill = PC1)) +
  theme_minimal() + scale_fill_viridis()+
  labs(fill = "Index", caption = "Source: US Census ACS 2016 estimates")+
